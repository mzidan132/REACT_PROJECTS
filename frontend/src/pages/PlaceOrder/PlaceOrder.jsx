...
const PlaceOrder = () => {
  ...
  const [data, setData] = useState({ //empty after order placed
    firstName: "",
    lastName: "",
    email: "",
    street: "",
  ...
  const onChangeHandler = (event) => {
    const name = event.target.name; //all item info value changing
  ...
  const placeOrder = async (event) => {
    ...
    let orderItems = [];
    food_list.map((item) => {
      if (cartItems[item._id] > 0) { //check item is in cart
        let itemInfo = item;
        itemInfo["quantity"] = cartItems[item._id]; //add item qauntatiy
        orderItems.push(itemInfo) //add item to orderItem
      }
    })
    let orderData = {
      address: data,
    ...
    if (response.data.success) {
      const { session_url } = response.data; //session url for stripe up backend
      window.location.replace(session_url); //redirect to stripe site
    }
    else {
      alert("Error");
    }
  ...
  useEffect(() => { //can create private route
